...
app.post('/api/auth/customer',async(req,res)=>{
  try{const {customerCode,password}=req.body; const r=(await (await db()).query('SELECT * FROM customers WHERE customer_code=$1',[String(customerCode||'').trim()])).rows[0];
    ...
    res.json({token,user:{role:'customer',name:r.name,customerCode:r.customer_code,year:r.year}});
...
app.post('/api/customer/order',auth,role('customer'),async(req,res)=>{
  ...
  try{
    ...
    const discount=Number(req.body.discount)>0 && calculatedSubtotal>=100 ? Math.round(calculatedSubtotal*0.10) : 0;
    const fee=3;
